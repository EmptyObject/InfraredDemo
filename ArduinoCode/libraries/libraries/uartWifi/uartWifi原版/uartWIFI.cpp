...
bool WIFI::begin(void)
{
	...
	DebugSerial.begin(DefaultBaudRate);		//Esp8266软串口波特率
	DebugSerial.flush();
	DebugSerial.setTimeout(10000);
	Serial.println("AT+RST");
	DBG("AT+RST\r\n");
	result = DebugSerial.find("OK");
	if(result){
...
bool WIFI::begin(long wifiBaudRate,long debugBaudRate)
{
	boolean result = false;
	DebugSerial.begin(wifiBaudRate);
	
	DebugSerial.begin(debugBaudRate);
	DebugSerial.flush();
	DebugSerial.setTimeout(10000);
	DebugSerial.println("AT+RST");
	result = DebugSerial.find("OK");
	if(result){
		Serial.println("Module is ready\r\n");
		return true;
...
void WIFI::Reset(void)
{
	...
	start = millis();
    while (millis()-start<5000) {                            
        if(DebugSerial.find("OK")==true)
        {
			Serial.println("reboot wifi is OK\r\n");
           break;
        }
    }
}
...
{
    String data;
	DebugSerial.print("AT+CWMODE=");
	DebugSerial.println(String(a));
	 unsigned long start;
	start = millis();
    while (millis()-start<2000) {
      if(DebugSerial.available()>0)
...
boolean WIFI::confJAP(String ssid , String pwd)
{
	...
	DebugSerial.print("\"");      //"pwd"
	DebugSerial.print(pwd);
	DebugSerial.println("\"");

	delay(5000);
	if (DebugSerial.find("OK"))
	{
...
boolean WIFI::confSAP(String ssid , String pwd , byte chl , byte ecn)
{
	...
	DebugSerial.print(",");
	DebugSerial.print(String(chl));

	DebugSerial.print(",");
	DebugSerial.println(String(ecn));
	delay(10000);
	if (DebugSerial.find("OK") == true)
	{
		return true;
...
{
	DebugSerial.println("AT+CIPSTATUS");
      String data;
    unsigned long start;
	start = millis();
    while (millis()-start<3000) {
...

{
	...
	DebugSerial.print("AT+CIPSTART=");
	DebugSerial.print("\"");
	DebugSerial.print(String(id));
	DebugSerial.print("\"");
    if(type>0)
    {
		DebugSerial.print("\"TCP\"");
    }
	else
    {
	...
	DebugSerial.print(",");
//    _cell.print("\"");
	DebugSerial.println(String(port));
//    _cell.println("\"");
    String data;
    unsigned long start;
	start = millis();
...
//    _cell.print("\"");
	DebugSerial.println(sizeof(str));
//    _cell.println("\"");*/
...
{
	DebugSerial.println("AT+CIPCLOSE");
...
{
	DebugSerial.print("AT+CIPCLOSE=");
	DebugSerial.println(String(id));
    String data;
    unsigned long start;
	start = millis();
	while (millis()-start<3000) {
...
{
	DebugSerial.print("AT+CIPSERVER=");
	DebugSerial.print(String(mode));
	DebugSerial.print(",");
	DebugSerial.println(String(port));
...
/*bool WIFI::ServerConnect(String ServerIP,String ServerPort)
{
	boolean result = false;
